...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 The inline joke is visually distinct from tool-call output in any terminal, via emoji + structure; colour only where proven to render
- [ ] #2 The entrypoint stdout contract (exit 0, newline-terminated valid JSON, no context leakage) is guarded by an automated test that fails on a dropped trailing newline
- [ ] #3 README documents every colour knob that exists and none that don't; NO_COLOR per no-color.org is honoured
<!-- AC:END -->
