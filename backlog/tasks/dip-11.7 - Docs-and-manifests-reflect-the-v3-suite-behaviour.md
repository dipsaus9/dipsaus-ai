...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 README documents auto-isolation, the empty-repo init bootstrap and the run-vs-deliver distinction, with no stale worktree.enabled reference remaining
- [ ] #2 .claude/CLAUDE.md Architecture reflects the removed config key and auto-isolation
- [ ] #3 The three manifest (package.json, .claude-plugin/plugin.json, .claude-plugin/marketplace.json) descriptions are consistent with v3
- [ ] #4 A grep for worktree.enabled across docs and manifests returns nothing
<!-- AC:END -->
