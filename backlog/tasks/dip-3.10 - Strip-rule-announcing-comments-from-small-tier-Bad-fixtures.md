...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 No detection-call source file (Bad.tsx and support files across fixtures/srp, fixtures/composition, fixtures/state) contains a comment naming a rule id or announcing its violation; verified by grep for 'Violates' and rule-id patterns with zero hits outside Good exemplars
- [ ] #2 Zero behavior change: island suite green, no component code modified, expected.json anchors re-pointed where line numbers shifted
- [ ] #3 Repo bun run lint/typecheck/test green
<!-- AC:END -->
